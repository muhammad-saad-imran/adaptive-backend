...
export interface IPolicy {
  ...
  data: {
    ...
    quoteEstimates: Array<IQuoteEstimate>;
    selectedEstimateId: string;
    productRaterVersion: string;
  };
  metaData: IMetaData;
  ...
  boundDateUtc?: string;
  documents: {
    quote: Array<IDocument>;
    policy: Array<IDocument>;
  };
  quoteId: string;
  status: string;
  policyNumberInc: number;
  policyNumber: string;
  historyData: Array<any>; // Depending on the structure of historyData, you can refine this further.
  isAscendTransactionSubmitted: boolean;
  transactionData: any; // Refine this based on the actual structure of transactionData.
  createdAt: string;
  updatedAt: string;
